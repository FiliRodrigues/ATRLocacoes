...
export const getLicenciamento: AtrTool = {
  name: "get_licenciamento",
  category: "read",
  description:
    "Consulta licenciamento anual por veículo. Use para perguntas sobre valores, vencimentos e status de pagamento do licenciamento.",
  input_schema: {
    type: "object",
    properties: {
  ...
  handler: async (input, ctx) => {
    ...
    let query = supabase.from("licenciamento")
    ...
    return {
      ...
      data: data || [],
      total: (data || []).length,
      message: `${(data || []).length} registro(s) de licenciamento encontrado(s).`,
    };
  },
};
